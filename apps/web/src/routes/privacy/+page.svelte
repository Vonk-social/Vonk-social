...
<svelte:head>
	<title>Vonk — {t('privacy.title', locale)}</title>
</svelte:head>
...
<article class="mx-auto max-w-3xl px-4 pt-16 pb-20">
	<header class="mb-10 flex flex-col items-center text-center">
		...
		<p class="mt-2 text-sm text-muted">{t('privacy.updated', locale)}</p>
	</header>

	<div class="vonk-card mb-6">
		<p class="text-muted">{t('privacy.intro', locale)}</p>
	</div>

	{#each sections as n}
		<section class="vonk-card mb-4">
			<h2 class="font-display text-lg font-bold text-ink">
				{t(`privacy.section${n}_title`, locale)}
			</h2>
			<p class="mt-2 text-muted">
				{t(`privacy.section${n}_body`, locale)}
			</p>
		</section>
	{/each}
